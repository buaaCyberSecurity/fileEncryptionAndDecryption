...
{
	unsigned char *blk_now = blk;
	int t = 0;
	for(;t<16;t++)	
	{
		AesEncrypt(blk_now,expansionkey,Nr);
		blk_now+=16;
	}
}
//AES 解密总函数
...
void Contrary_256_AesEncrypt(unsigned char *blk,unsigned char *expansionkey,int Nr)
{
	...
	for(;t<16;t++)	
	{
		Contrary_AesEncrypt(blk_now,expansionkey,Nr);
		blk_now+=16;
	}
}
...
//~ int main(void){
	...
	//~ Aes_256_encrypt(pt,expansionkey,10);		//2、AES 加密
	...
	//~ /*解密*/
	//~ Contrary_256_AesEncrypt(pt,expansionkey,10);//AES 解密
